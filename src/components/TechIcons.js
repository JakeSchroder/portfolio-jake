...
export default function TechIcons({theme}){
    ...
    return(
        <Grid item xs={1} display="flex" justifyContent="space-between" direction='column' position='relative' sx={{left:-40, paddingBottom:5, paddingRight:20}}>
            ...
            <Button href='https://reactjs.org/' target='_blank' position='relative' sx={{left: -70, top: -25, backgroundColor:'transparent', animation: pulsateFwdAnime, animationDelay:'4s'}}>
                <ReactIcon fill={theme.palette.secondary.light} />
            </Button>
            <Button href='https://www.mongodb.com/' target='_blank' position='relative' sx={{left: -20, backgroundColor:'transparent', animation: pulsateFwdAnime}}>
                <MongoIcon fill={theme.palette.secondary.light} />
            </Button>
            <Button href='https://nodejs.org/en/' target='_blank' position='relative' sx={{left: -70, top: 30, backgroundColor:'transparent', animation: pulsateFwdAnime, animationDelay:'.75s'}}>
                <NodeIcon fill={theme.palette.secondary.light} />
            </Button>
            <Button href='https://www.nginx.com/' target='_blank' position='relative' sx={{left: -170, top: 60, backgroundColor:'transparent', animation: pulsateFwdAnime, animationDelay:'3s'}}>
